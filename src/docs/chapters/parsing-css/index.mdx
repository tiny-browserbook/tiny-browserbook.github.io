...
そこで本章では、以下のようなスタイルシートを処理することを目指します:
...
- 全ての style rule について、以下が成り立つ。
    ...
    - declaration list の中には [CSS Display Module Level 3](https://drafts.csswg.org/css-display/#the-display-properties) で定義されている `display` プロパティのみを持つ。
